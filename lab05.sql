...
--10.
GO
DECLARE @highest AS real = (SELECT MAX(SoNhanVien) FROM (SELECT MaPB, COUNT(*)[SoNhanVien] FROM NhanVien GROUP BY MaPB) A INNER JOIN PhongBan
ON A.MaPB = PhongBan.MaPB)
SELECT A.MaPB, TenPB, SoNhanVien FROM
(SELECT MaPB, COUNT(*)[SoNhanVien] FROM NhanVien GROUP BY MaPB) A INNER JOIN PhongBan
ON A.MaPB = PhongBan.MaPB WHERE SoNhanVien=@highest
--11.
GO
...
--15.
GO
DELETE FROM NhanVien WHERE NOT EXISTS(SELECT MaNV FROM LuongDA WHERE MaNV= NhanVien.MaNV)