...
SELECT OrderId,SUM(Price*qty) [Total Price] FROM(SELECT /*Product.ProductId, qty,Price,OrderId*/ * FROM
(SELECT * FROM OrderDetails ) a INNER JOIN Product ON a.ProductId = Product.ProductId ) b GROUP BY OrderId